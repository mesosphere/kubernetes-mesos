...
//TODO(karl) move command construction into ExecutorClient (separate url construction from listeners)
func (s *SchedulerServer) prepareExecutorCommand(hks hyperkube.Interface) (*mesos.CommandInfo, error) {
	...
	if s.ExecutorPath != "" {
		...
		return nil, fmt.Errorf("either run this scheduler via km or else --executor_path is required")
	} else {
	...
	if s.ProxyPath != "" {
		...
		return nil, fmt.Errorf("either run this scheduler via km or else --proxy_path is required")
	} else if s.ExecutorPath != "" {
		return nil, fmt.Errorf("proxy can only use km binary if executor does the same")
...
func (s *SchedulerServer) bootstrap(hks hyperkube.Interface, sc *schedcfg.Config) (*ha.SchedulerProcess, ha.DriverFactory, tools.EtcdGetSet, uid.UID) {
	...
	executorCmd, err := s.prepareExecutorCommand(hks)
	if err != nil {
	...
	kpl := scheduler.NewPlugin(mesosPodScheduler.NewPluginConfig(schedulerProcess.Terminal(), s.mux, nil))
	...
	return schedulerProcess, driverFactory, etcdClient, executor.ID()
}
